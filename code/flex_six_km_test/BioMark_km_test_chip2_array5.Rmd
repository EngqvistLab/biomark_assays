...
FAM_DATA <- '1530024054_chip_2_array_5_fam.tsv'
ROX_DATA <- '1530024054_chip_2_array_5_rox.tsv'
...
ggplot(in_data, aes(x=final_substrate_conc, y=activity))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Activity, chip %s, array %s, background-subtracted data', CHIP_NUMBER, ARRAY_NUMBER)) +
...
ggplot(final.data.filtered.both.no_outliers.df, aes(x=final_substrate_conc, y=activity_moles_protein))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Selected linear range, chip %s, array %s, outliers removed', CHIP_NUMBER, ARRAY_NUMBER)) +
    xlab(sprintf('Substrate conentration (%s)', unique(in_data$final_substrate_unit))) + 
    ylab(sprintf('Activity (%s)', unique(in_data$activity_moles_protein_unit))) +
    facet_grid(cols=vars(sample_substrate), rows=vars(assay_enzyme, final_enzyme_conc))
...
```{r}
write.table(final.data.filtered.both.no_outliers.df, file=paste(FINAL, THIS_PROJECT_FOLDER, sprintf('%s_flex_six_chip_%s_array_%s_final_data.tsv', gsub(' ', '_', unique(all.df$assay_enzyme)), CHIP_NUMBER, ARRAY_NUMBER), sep='/'), sep='\t', row.names=FALSE)
```
...
```{r}
final.data.filtered.both.no_outliers.df <- final.data.filtered.both.no_outliers.df[final.data.filtered.both.no_outliers.df$activity_moles_protein <10, ]
