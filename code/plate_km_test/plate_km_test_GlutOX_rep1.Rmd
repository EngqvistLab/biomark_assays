...
Experimental data collected on ?????? (DDMMYY) in in a plate reader.
...
```{r}
THIS_PROJECT_FOLDER <- 'plate_km_test'
REPEAT <- 1
ROX_DATA <- 'GlutOX_data_rep1.tsv'
FAM_DATA <- 'zero_fam_data.tsv'
...
SLOPE_FIRST_TIMEPOINT <- 1
SLOPE_LAST_TIMEPOINT <- 12
...
```{r}
assay.df <- read.table(paste(INTERMEDIATE, THIS_PROJECT_FOLDER, 'GlutOX_assay.tsv', sep='/'), sep='\t', header=TRUE, na.strings="")

sample.df <- read.table(paste(INTERMEDIATE, THIS_PROJECT_FOLDER, 'GlutOX_sample.tsv', sep='/'), sep='\t', header=TRUE, na.strings="")
...
```{r}
write.table(all.df, file=paste(FINAL, THIS_PROJECT_FOLDER, sprintf('%s_384-well_plate_rep%s_final_data.tsv', gsub(' ', '_', unique(all.df$assay_enzyme)), REPEAT), sep='/'), sep='\t', row.names=FALSE)
```
...
```{r}
concentration.df <- fluorescence_to_concentration(all.df, chip_type='plate')
...
ggplot(in_data, aes(x=final_substrate_conc, y=activity))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Activity, repeat %s, background-subtracted data', REPEAT)) +
    xlab(sprintf('Substrate concentration (%s)', unique(in_data$final_substrate_unit))) + 
...
```{r}
final.data.moles.df <-concentration_to_moles(final.data.df, chip_type='plate')
...
```{r}
final.data.moles.protein.df <- normalize_to_protein(final.data.moles.df, chip_type='plate')
...
ggplot(in_data, aes(x=final_substrate_conc, y=activity_moles_protein))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Activity, repeat %s', REPEAT)) +
    xlab(sprintf('Substrate concentration (%s)', unique(sample.df$sample_substrate_unit))) + 
...
ggplot(in_data, aes(x=final_substrate_conc, y=activity_moles_protein))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Selected linear range, repeat %s, filtered data', REPEAT)) +
    xlab(sprintf('Substrate concentration (%s)', unique(sample.df$sample_substrate_unit))) + 
    ylab(sprintf('Activity (%s)', unique(in_data$activity_moles_protein_unit))) +
    facet_grid(cols=vars(sample_substrate), rows=vars(assay_enzyme, final_enzyme_conc))
...
ggplot(final.data.filtered.both.no_outliers.df, aes(x=final_substrate_conc, y=activity_moles_protein))+
    theme_bw() +
    geom_point(cex=0.5) + 
    ggtitle(sprintf('Selected linear range, repeat %s, outliers removed', REPEAT)) +
    xlab(sprintf('Substrate concentration (%s)', unique(in_data$final_substrate_unit))) + 
    ylab(sprintf('Activity (%s)', unique(in_data$activity_moles_protein_unit))) +
    facet_grid(cols=vars(sample_substrate), rows=vars(assay_enzyme, final_enzyme_conc))
...
```{r}
write.table(final.data.filtered.both.no_outliers.df, file=paste(FINAL, THIS_PROJECT_FOLDER, sprintf('%s_384-well_plate_rep%s_final_data.tsv', gsub(' ', '_', unique(all.df$assay_enzyme)), REPEAT), sep='/'), sep='\t', row.names=FALSE)
```
...
p1 <- ggplot(in_data, aes(x=final_substrate_conc, y=activity_moles_protein))+
    ...
    ggtitle(sprintf('Km plot, 384-well plate repeat %s', REPEAT)) +
...
# save the figure
fname = paste(FIGURES, sprintf('%s_384-well_plate_rep%s_final_data.pdf', gsub(' ', '_', unique(all.df$assay_enzyme)), REPEAT), sep='/')
