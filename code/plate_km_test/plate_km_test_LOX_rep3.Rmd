...
TOLERANCE_AT_VMAX <- 0.15

SLOPE_FIRST_TIMEPOINT <- 0
SLOPE_LAST_TIMEPOINT <- 3
...
Subtract fluorescence of the no enzyme control

```{r}
# remove fluorescence of the no enzyme control
...
Convert fluorescence to concentration of product using standard curve ### block changed ###
...
ggplot(in_data, aes(x=minutes, y=resorufin_value_norm, color=factor(final_substrate_conc)))+
    ...
    ggtitle('Raw data measured over time') +
    xlab('Time (minutes)') + 
    ylab(sprintf('Resorufin produced (%s)', unique(in_data$resorufin_value_norm_unit))) +
    facet_grid(cols=vars(sample_substrate), rows=vars(assay_enzyme, final_enzyme_conc))
```
...
Red indicates samples removed because no activity was detected (low substrate and enzyme).
...
```{r}
final.data.moles.df <- concentration_to_moles(bundled.slopes.nonegs.spearman.max.filtered.unbundled.df, chip_type='plate')
...
```{r}
write.table(final.data.filtered.both.df, file=paste(FINAL, THIS_PROJECT_FOLDER, sprintf('%s_384-well_plate_rep%s_final_data.tsv', gsub(' ', '_', unique(all.df$assay_enzyme)), REPEAT), sep='/'), sep='\t', row.names=FALSE)
```
