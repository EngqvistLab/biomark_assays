...
```{r}
THIS_PROJECT_FOLDER <- 'flex_six_second_km_test'
PLATE_NUMBER <- 3
ARRAY_NUMBER <- 3
DATA_FOLDER <- sprintf('Plate_%s_Array_%s', PLATE_NUMBER, ARRAY_NUMBER)
FAM_DATA <- '1530024086_plate_3_array_3_fam.tsv'
ROX_DATA <- '1530024086_plate_3_array_3_rox.tsv'
...
bundled.slopes.nonegs.spearman.max.filtered.unbundled.df <- unbundle_data(bundled.slopes.nonegs.spearman.df)
...
# plot it
in_data <- bundled.slopes.nonegs.spearman.max.filtered.unbundled.df
ggplot(in_data, aes(x=final_substrate_conc, y=slope))+
    theme_bw() +
    geom_point(cex=0.5) + 
